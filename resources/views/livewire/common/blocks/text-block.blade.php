<div class="relative bg-white" :wire:key="$document->id">
    ...
    <div class="relative">
        ...
        <div class="relative group/block">
            <textarea class="focus:bg-blue-100 focus:cursor-text cursor-pointer p-0 text-zinc-700 autoExpandTextarea w-full border-0 text-xl group-hover/block:bg-red-100 rounded" name="text" rows="1" wire:model.debounce.500ms="content" wire:ignore></textarea>
    ...
    <x-experior::modal>
        <div class="py-4 text-left px-6">
            ...
            <input placeholder="ie: Rewrite this text but with a funnier tone" class="w-full text-base border-0 bg-gray-100 p-0 rounded-xl py-3 px-4" name="text" wire:model="customPrompt" />
            @if ($errors->has('customPrompt'))
            <span class="text-red-500 text-sm">{{ $errors->first('customPrompt') }}</span>
            @endif
            <div class="flex justify-start mt-8">
                <button wire:click="runCustomPrompt" wire:loading.remove class="flex items-center gap-4 bg-secondary text-xl hover:bg-main text-white font-bold px-4 py-2 rounded-xl">
