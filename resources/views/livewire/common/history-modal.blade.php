<x-experior::modal>
    <div class="py-4 text-left px-6">
                ...
                @foreach ($history as $key => $item)
                    <div class="flex flex-col gap-3 rounded-lg border border-zinc-200 p-2">
                        <div class="flex items-center justify-between">
                            ...
                            @endif
                            @if ($key > 0)
                                <x-button sm class='rounded-lg text-white bg-secondary hover:bg-black'
                                    wire:click="apply('{{ base64_encode($item['content']) }}')" label="Apply" />
                            @endif
                        </div>
        ...
        <div class="flex justify-center pt-4 z-30">
            <x-button label='Close' slate wire:click="$emitUp('closeHistoryModal')" class="py-2 px-4 rounded-lg"
                type="button" />
        </div>
    </div>
</x-experior::modal>
