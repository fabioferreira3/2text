    ...
    @if(!$menuOpen)
        <div class="text-zinc-600 overflow-auto flex flex-col absolute z-40 top-12 right-0 border rounded-lg w-64 text-sm bg-zinc-100">
            ...
            <div class="flex flex-col gap-1 px-4 py-2">
                <button class="flex items-center gap-2 bg-secondary border border-secondary hover:bg-zinc-200 hover:text-zinc-700 hover:border hover:border-zinc-300 py-1 px-3 rounded-lg text-sm text-white w-full" wire:click='toggle'>
                    <x-icon class="w-5 h-5" name="volume-up"/>
                    <div class="font-bold">{{__('common.listen')}}</div>
                </button>
                <button class="flex items-center gap-2 bg-main border border-main hover:bg-zinc-200 hover:text-zinc-700 hover:border hover:border-zinc-300 py-1 px-3 rounded-lg text-sm text-white w-full" wire:click='toggle'>
