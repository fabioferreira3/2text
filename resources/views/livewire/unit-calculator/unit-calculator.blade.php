<div class="relative">
    <button wire:click="$emit('closeUnitCalculator')" class="flex items-center h-10 w-10 rounded-full gap-1 absolute top-2 right-8 bg-gray-300 p-2 font-bold text-gray-800">
    ...
    <div class="flex flex-col p-8">
        ...
        <div class="flex flex-col gap-8 items-center">
            <div class="flex flex-col items-center gap-4">
                ...
                <input type="number" min="0" max="10000" wire:model="units" class="border border-gray-300 text-center rounded-lg focus:outline-none focus:ring-2 focus:ring-[#EA1F88] text-lg text-gray-600 px-3">
                @if($errors->has('units'))
                <span class="text-red-500 text-sm">{{ $errors->first('units') }}</span>
                @endif
                <button wire:click="processPurchase" class="bg-secondary px-4 py-1 rounded-xl text-white font-bold">Buy</button>
            </div>
            <div class="flex flex-col items-center w-1/2 justify-center gap-4 text-center border border-gray-300 rounded-xl p-8">
